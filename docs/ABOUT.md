...
- **MCP server for AI agents** _(free)_ - A stdio MCP server exposes 42 tools (projects, process start/stop/restart/enable/disable/all, logs, error log, threshold alerts, live browser tabs of a dev app) as a thin client over the same running daemon the GUI uses, on the shared kit-synced MCP engine that now dispatches requests concurrently (bounded by maxInFlight, default 8) so a slow tool call no longer blocks a fast ping or tools/list queued behind it on the same connection. - `server/src/mcp.ts`, `server/src/mcp-stdio.mjs`
