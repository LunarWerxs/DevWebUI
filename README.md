...
41 tools cover projects, processes (start/stop/restart, enable/disable, all), logs, the error
...
maintained, free, and local-first, and pairs its GUI with a 41-tool MCP server so AI agents can
drive the same daemon you click.

**Can AI agents control DevWebUI directly?**
Yes. DevWebUI ships a stdio MCP server (`devwebui mcp`, or `server/src/mcp.ts`) with 41 tools
